...
contract CCIPTokenSender is Ownable {
    ...
    /// @notice Thrown when msg.value is less than the CCIP fee.
    ...
    event DestinationChainAllowlistUpdated(uint64 indexed chainSelector, bool allowed);
    ...
    mapping(uint64 chainSelector => bool allowed) public allowlistedDestinationChains;
    ...
    function setDestinationChainAllowlist(uint64 chainSelector, bool allowed) external onlyOwner {
    ...
    function sendTokens(
        ...
        if (msg.value < fee) {
            revert InsufficientNativeFee(fee, msg.value);
        }
        ...
        // Dispatch the CCIP message.
        messageId = i_router.ccipSend{value: fee}(destinationChainSelector, message);

        emit TokensSent(messageId, destinationChainSelector, receiver, token, amount, fee);
    }
